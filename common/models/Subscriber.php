...
//            SubscriberTransaction::CHANNEL_TYPE_WAP => 'Wap',
            ...
            SubscriberTransaction::CHANNEL_TYPE_ANDROID => Yii::t('app', 'Android'),
...
//                            $this->cancelServicePackage($service, $channel_type, $transaction_type);
        ...
        if ($chargingSuccess) {
            ...
            if (isset($cancelServicePackages)) {
                foreach ($cancelServicePackages as $service) {
                    $this->cancelServicePackage($service, SubscriberTransaction::CHANNEL_TYPE_CSKH, SubscriberTransaction::TYPE_CANCEL, 0, false);
                }
            }
            $err_code = CommonConst::API_ERROR_NO_ERROR;
    ...
    )
    {
        ...
        if ($chargingSuccess) {
            ...
            return array(
                "error" => CommonConst::API_ERROR_CHARGING_FAIL,
                "message_web" => \Yii::t('app', 'Tài khoản không đủ'),
                "message" => \Yii::t('app', 'Tai khoan khong du'),
            );
        }
    }
...
//        $transactionID = (isset($tr) ? $tr->id : null);
            if ($chargingSuccess) {
                ...
                return array(
                    "error" => CommonConst::API_ERROR_NO_ERROR,
                    "message_web" => \Yii::t('app', 'Thành công!'),
                    "link_download" => $url,
                );
            } else {
                return array(
...
//            $res = $vas_provisioning->vasRegisterPackage($this->msisdn, $service, $transaction->id, $channel, $day_promotion, $promotion_note);
    ...
    public function coreBuyPackage($package, $transaction, $channel, $promotion = 0, $trial = 0, $sms = true)
    {
        $res = [];
        $res['success'] = false;
        $res['message'] = \Yii::t('app', 'Thất bại');
        ...
        if (empty($transaction->description)) {
            $transaction->description = empty($promotion_note) ? \Yii::t('app', "Mua goi cuoc $package->name") : $promotion_note;
        }
        /**
        ...
        if (!$is_registered) {
            ...
            if ($promotion > 0 || $trial > 0) {
                ...
                $promotion_note = \Yii::t('app', "Khuyen mai theo yeu cau ben vinaphone");
            } else {
                /**
                ...
                if ($is_promotion) {
                    $real_price = 0;
                    $day_promotion = $package->free_days;
                    $promotion_note = \Yii::t('app', "Khuyen mai trong chu ky khuyen mai");
                } else {
...
//            ->chargeRegisterPackage(
    ...
    public function coreCancelPackage($package, $transaction, $channel, $sms = false)
    {
        ...
        $transaction->msisdn = $this->msisdn;
        if (empty($transaction->description)) {
            $transaction->description = \Yii::t('app', "Huy goi cuoc $package->name");
        }
        ...
        if ($cancel_package) {
            ...
            $transaction->description = \Yii::t('app', "Huy goi dich vu '") . $package->name . "'";
            // them transaction
...
//                $expiryDate->add(DateInterval::createFromDateString($purchasePackage->period . ' days'));
    ...
    public static function sendMail($to_mail, $subject, $content, $file_attach)
    {
    ...
    {
        if ($channel_id == Subscriber::CHANNEL_TYPE_ANDROID) {
            $channel_type = 'Smartbox';
        } elseif ($channel_id == Subscriber::CHANNEL_TYPE_WEBSITE) {
    ...
    public static function getErrorCode($error_code)
    {
        switch ($error_code) {
...
//                return Yii::t('app', 'Lỗi chữ ký dữ liệu');
    ...
    public static function getErrorCodePhone($error_code)
    {
        switch ($error_code) {
    ...
    public static function GetDataResult($result)
    {
        $data = [];
    ...
    public static function GetCodeResult($result)
    {
        $chuoi = explode(',', $result);
    ...
    public static function AddSessionID($user_login, $pass)
    {
    ...
    public static function writeLog($id)
    {
        FileUtils::appendToFile(Yii::getAlias('@runtime/logs/sessionId.log'), $id);
    }

    public static function ReturnSessionId($log_link)
    {
    ...
    public static function SaveTransactionVoucherPhone($card_code, $card_serial, $nn, $subscriber, $message, $error_code, $status, $type, $cost, $balance, $id_t, $site_id)
    {
        $st = new SubscriberTransaction();
        $st->status = $status;
        $st->card_code = $card_code;
        $st->card_serial = $card_serial;
        $st->msisdn = $subscriber->msisdn;
        $st->description = $message;
        $st->site_id = $site_id;
        $st->error_code = $error_code;
        $st->type = $type;
        $st->channel = SubscriberTransaction::getChanelTypeVoucher($nn);
        $st->transaction_voucher_id = $id_t;
        $st->subscriber_id = $subscriber->id;
        $st->cost = $cost;
        $st->balance = $balance;
        $st->currency = 'VND';
        $st->transaction_time = time();
        $st->created_at = time();
        if ($st->save()) {
            return true;
        } else {
            Yii::error($st->getErrors());
            return false;
        }
    }

    public static function CheckUserVoucherPhone($subscriber, $nn, $site_id)
    {
    ...
    public static function CheckUserVoucher($subscriber, $site_id)
    {
        $time_now = (new DateTime('now'))->format('Y-m-d H:i:s');
        $time_30i = (new DateTime('now'))->modify('-30 minutes')->format('Y-m-d H:i:s');
        $now_integer = strtotime($time_now);
    ...
    public static function UpdateTransactionVoucher($subscriber, $error_code, $mes, $cost, $balance, $site_id, $card_code, $status, $tr_id = null)
    {
        $st = new SubscriberTransaction();
        $st->site_id = $site_id;
        $st->msisdn = $subscriber->msisdn;
        ...
        $st->status = $status;
        $st->subscriber_id = $subscriber->id;
        $st->transaction_voucher_id = $tr_id;
        $st->channel = SubscriberTransaction::CHANNEL_TYPE_VOUCHER;
        $st->type = SubscriberTransaction::TYPE_VOUCHER;
        $st->error_code = $error_code; // 00
        $st->cost = $cost;
        $st->description = $mes;
        $st->balance = $balance;
        $st->currency = 'VND';
    ...
    public static function validateTopup($name_subscriber, $cost, $partner, $signature,$type = false)
    {
        if($type == false){
            if (empty($cost)) {
        ...
        $md5 = md5($name_subscriber . $cost . $partner . $key); Yii::info('Ma md5 dung '.$md5);
//        echo"<pre>";print_r($md5);die();
    ...
    public static function getSubscriberTopup($name_subscriber, $cost)
    {
        $subscriber = Subscriber::findOne(['username' => $name_subscriber]);
        if (!isset($subscriber)) {
        ...
        if ($subscriber->save()) {
            $trans = $subscriber->newTransaction(SubscriberTransaction::TYPE_VOUCHER,
                SubscriberTransaction::CHANNEL_TYPE_VOUCHER_PARTNER, 'Nạp tiền từ đại lý',
                null, null, SubscriberTransaction::STATUS_SUCCESS, $cost, 'VND', $subscriber->balance,self::TOPUP_SUCCESS);
            if (!$trans) {
    ...
    public static function saveSubscriber($subscriber, $otp)
    {
        /** @var Subscriber $subscriber */
        $subscriber = Subscriber::findOne(['id' => $subscriber->id]);
        $subscriber->otp_code = $otp;
        $subscriber->expired_code_time = time() + 3 * 60;
        $subscriber->number_otp = 3;
