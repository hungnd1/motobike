    ...
    */
    ...
    public function actionGetUrl($id, $quality)
    {
        ...
        if ($content->getIsFree($this->site->id) == Content::NOT_FREE) {
            ...
            if (!$purchase) {
                throw new NotFoundHttpException(Yii::t('app', 'Access Denny. Token hết phiên truy cập, vui lòng login lấy token mới để thao tác với hệ thông.'));
            }
        }
        ...
        $res = Content::getUrl($type_check, $contentProfile, $this->site->id);
        if (!$res['success']) {
...
//        if(!$cssa){
//            throw new ServerErrorHttpException(Message::getNotFoundContentMessage());
//        }
...
//            }
//            /** Hiển thị message update lỗi chưa phân phổi đủ chất lượng video */
//            throw new ServerErrorHttpException(Message::getTranferErrorMessage());
//        }
...
//    }
    ...
    {
        $is_check_count = Content::checkCountQualityWhenDownload(77, 6);
        if (!$is_check_count) {
            CUtils::log('#### actionSyncContentToSite Downloader: ' . Message::getTranferErrorMessage());
            throw new ServerErrorHttpException(Message::getTranferErrorMessage());
        }
    }
}