...
<div class="form-body">
    ...
    <div class="row">
        <div class="col-md-12">
            <?= $form->field($model, 'category_id')
                ->dropDownList(\yii\helpers\ArrayHelper::map(\common\models\Category::find()
                    ->andWhere(['type' => \common\models\Category::TYPE_QA])->asArray()->all(), 'id', 'display_name'),
                    ['prompt' => ' -- Chọn danh mục --']) ?>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <?= $form->field($model, 'answer')->widget(CKEditor::className(), ['options' => ['rows' => 8],
            'preset' => 'full']) ?>
        </div>
    </div>
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <?= $form->field($model, 'image')->widget(FileInput::classname(), ['options' => ['multiple' => true, 'accept' => 'image/*'],
